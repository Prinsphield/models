...
class Delg(delf_clip_model.Delf):
  ...
  def __init__(self,
    ...
    Args:
      ...
      text_trainable: bool, whether the text classifier is trainable or fixed.
      text_scale: float, the text scale factor.
      text_weight_ckpt: the ckpt path to the text classifier weight.
    """
    ...
    self._text_weight_ckpt = text_weight_ckpt
    self._text_trainable = text_trainable
  ...
  def init_classifiers(self, num_classes):
    ...
    text_classifier_func = self._create_text_classifier(num_classes, self._text_trainable)
    super(Delg, self).init_classifiers(
        num_classes,
        desc_classification=backbone_classifier_func,
        text_classification=text_classifier_func)
  ...
  def _create_text_classifier(self, num_classes, trainable):
    """Define the text classifier."""
    logging.info('Creating text classifier')
    if self._text_weight_ckpt:
      text_weight_init = np.load(self._text_weight_ckpt).transpose()
    else:
      text_weight_init = tf.initializers.GlorotUniform()(
          shape=[self._embedding_layer_dim, num_classes])
    self.text_weights = tf.Variable(
        initial_value=text_weight_init,
        name='text_weights',
        trainable=trainable)

    text_classifier_func = functools.partial(text_classifier_logits,
                                             text_weights=self.text_weights,
                                             scale_factor=self._text_scale)
    text_classifier_func.trainable_weights = [self.text_weights]
    return text_classifier_func


def text_classifier_logits(prelogits, text_weights, scale_factor):
  """Compute text classifier logits.

  Args:
    prelogits: float tensor of shape [batch_size, embedding_layer_dim].
    num_classes: int, number of classes.
    text_weights: float tensor of shape [embedding_layer_dim, num_classes].
    scale_factor: float.

  Returns:
    logits: Float tensor [batch_size, num_classes].
  """
  # L2-normalize prelogits and then obtain cosine similarity
  normalized_prelogits = tf.math.l2_normalize(prelogits, axis=1)
  normalized_weights = tf.math.l2_normalize(text_weights, axis=1)
  cosine_sim = tf.matmul(normalized_prelogits, normalized_weights)
  logits = cosine_sim * scale_factor
  return logits
