...
class Adapter(tf.keras.Model):
  ...
  def __init__(self, output_dim, name='adapter'):
    super(Adapter, self).__init__(name=name)
    self.output_dim = output_dim

    self.layer1 = layers.Dense(
        output_dim, activation=tf.keras.activations.relu, kernel_regularizer=None, name='layer1')
    self.layer2 = layers.Dense(
        output_dim, activation=None, kernel_regularizer=None, name='layer2')
...
class Delf(tf.keras.Model):
  ...
  def init_classifiers(self, num_classes, desc_classification=None,
    ...
    if text_classification is None:
      self.text_classification = layers.Dense(
          num_classes, activation=None, kernel_regularizer=None, name='text_fc')
    else:
  ...
  def global_and_local_forward_pass(self, images, training=True):
    ...
    text_prelogits = self.adapter(desc_prelogits)
    return (desc_prelogits, text_prelogits, attn_prelogits, attn_scores, backbone_blocks,
            dim_expanded_features, dim_reduced_features)

  def build_call(self, input_image, training=True):
    (global_feature, text_feature, attn_scores, backbone_blocks, _,
    ...
    return global_feature, text_feature, attn_scores, features

  def call(self, input_image, training=True):
    _, _, probs, features = self.build_call(input_image, training=training)
